defmodule Telluride.Messaging.PipelineThroughputConsumer do
  use GenServer
  use AMQP

  alias Telluride.Messaging.AMQPConnectionManager
  alias Telluride.Messaging.PipelineThroughputQueue
  ...
  def handle_cast({:channel_available, channel}, _state) do
    :ok = PipelineThroughputQueue.register_consumer(channel)
  ...
  def handle_info({:quit, reason}, %{channel: channel, consumer_tag: consumer_tag} = state) do
     PipelineThroughputQueue.cancel_consumer(channel, consumer_tag)
    {:stop, reason, state}
  end
  ...
  defp consume(channel, delivery_tag, _redelivered, payload) do
    case Jason.decode(payload) do
      {:ok, event_info} ->
        Telluride.PubSub.PipelineThroughputTopic.publish(event_info)
